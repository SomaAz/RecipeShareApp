...
class ProfileView extends GetWidget<ProfileViewController> {
  final String uid;
  const ProfileView(this.uid, {Key? key}) : super(key: key);
  @override
  ...
  Future<void> getData(ProfileViewController controller) async {
    try {
      controller.userData = await FirestoreService().getUserData(uid);
      controller.postedRecipes =
          await FirestoreService().getAllPostedRecipesOfUser(uid);
    } catch (e) {
      print(e);
    }
  }
